...
- [ ] Task 1: Rename enum variants in `ComparisonSessionState`
...
- [ ] Task 2: Rename methods and update assert messages
  - File: `domain/src/session/comparison_session.rs`
  - Action:
...
- [ ] Task 3: Update all remaining references in domain code and comments
  - File: `domain/src/session/comparison_session.rs`
  - Action:
...
- [ ] Task 4: Update test code in domain crate
  - File: `domain/src/session/comparison_session.rs`
...
- [ ] Task 5: Update web crate references
...
- [ ] Task 6: Verify code compiles and tests pass
...
- [ ] Task 9: Update `ux-design-specification.md`
...
- [ ] AC1: Given the domain crate, when `cargo test -p domain` is run, then all tests pass with the renamed enum variants and methods.
- [ ] AC2: Given the full workspace, when `cargo clippy` is run, then no warnings or errors are produced.
...
- [ ] AC5: Given the four planning docs (`epics.md`, `architecture.md`, `ux-design-specification.md`, `project-context.md`), when grepping for `Note1`, `Note2`, `playingNote1`, `playingNote2`, `on_note1`, or `on_note2`, then zero matches are found.
- [ ] AC6: Given historical docs (`docs/implementation-artifacts/1-*.md`, `docs/claude-audit/`, `docs/ios-reference/`), when comparing to their git-committed state, then zero changes have been made (no historical docs modified).
