...
- All runtime asset fetches in Rust code must use relative paths (`./soundfont/...`, `./soundfont/Samples.sf2`), never root-absolute (`/soundfont/...`). Trunk only rewrites paths in HTML, not in compiled WASM. Relative paths resolve against the `<base href>` and work correctly under any base path.
