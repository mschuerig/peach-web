...
pub fn PitchDiscriminationView() -> impl IntoView {
    ...
    // Cleanup on component unmount — stop training AND remove all event listeners
    {
        ...
        on_cleanup(move || {
            let (cancelled, terminated, session, note_player, audio_ctx, keydown_fn, visibility_fn) =
                &*cleanup_state;
            terminated.set(true);
            cancelled.set(true);
            help_paused.set(false);
    ...
    view! {
        <div class="pt-4 pb-12">
            <NavBar title=discrimination_title show_back=true on_back=on_nav_away_cb pill_group=true>
